...
const getDatasetRaw = ( langs: string[] ): DatasetRaw => {
  ...
  forEachLine ( csv, line => {
    ...
    if ( datasetRaw[lang]?.length >= DATASET_LIMIT ) return; // Already parsed enough sentences
